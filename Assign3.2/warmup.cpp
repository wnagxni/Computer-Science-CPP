...
int buggyCount(Vector<int> &v, int index, int sumSoFar) {
    if (index == v.size()) {
        ...
        return buggyCount(v, index + 1, sumSoFar += v[index]) //+=为赋值操作,本函数栈帧中sumSoFar的值已经修改为`sumSoFar + v[index]`!!!
             + buggyCount(v, index + 1, sumSoFar); //这里的sumSoFar已经变了!
    }
}
