...
/* * * * * * My Test Cases * * * * * */
STUDENT_TEST("避免负数导致无限递归") {
    EXPECT_EQUAL(factorial(-3), -1);
}

//STUDENT_TEST("myPower(), generated inputs") {
//    for (int base = -5; base < 25; base++) {
//        for (int exp = -7; exp < 10; exp++) {
//            cout << "\n base = " << base << ", exp = " << exp << endl;
//            EXPECT_EQUAL(myPower(base, exp), pow(base, exp));
//        }
//    }
//}

//STUDENT_TEST("myPower(), 固定输入确认错误") {
//    EXPECT_EQUAL(myPower(-2, 3), pow(-2, 3));
//    EXPECT(myPower(-2, -3) == pow(-2, -3));
//    EXPECT(myPower(-2, 4) != pow(-2, 4));
//    EXPECT(myPower(-2, -4) != pow(-2, -4));
//}

STUDENT_TEST("浮点比较不准1e-10") {
    EXPECT_EQUAL(myPower(-10, -10), pow(-10, -10));
    EXPECT_EQUAL(myPower(-10, -20), pow(-10, -20));
    EXPECT_EQUAL(myPower(-10, -30), pow(-10, -30));
